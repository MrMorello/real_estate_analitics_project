    ...
    for full_link in full_links:
        ...
        if request.status_code == 200:
            ...
            try:
                ...
                total_square = int(square.split("/")[:1][0])
                living_square = int(square.split("/")[:2][1])
                kitchen_square = int(square.split("/")[:3][2])
                current_floor = int(structure[5].text.split("/")[:1][0])
                number_of_floors = int(structure[5].text.split("/")[:2][1])
                build_year = int(structure[6].text)
                material = structure[7].text
                ceiling_height = Decimal(structure[8].text.split(" ")[:1][0].replace(',','.'))
                adv_list.append({
                    'price': price,
                    'district': district,
                    'address': address,
