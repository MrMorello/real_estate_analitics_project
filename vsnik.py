...
svflat = pd.read_excel('tables\SvNikFlat.xlsx', index_col=0)
...
svflat['avg_sqrm_living_room'] = svflat['living_square'] / svflat['number_of_rooms']
svflat['living_to_total'] = svflat['living_square'] / svflat['total_square']
svflat['kitchen_to_living'] = svflat['kitchen_square'] / svflat['living_square']
svflat['kitchen_to_total'] = svflat['kitchen_square'] / svflat['total_square']
svflat['curFlor_to_totFlor'] = svflat['current_floor'] / svflat['number_of_floors']
svflat['house_age'] = 2019 - svflat['build_year']
...
svflat_cat = svflat[['district', 'material', 'number_of_rooms']].copy()
...
svflat_cat = encoder.transform(svflat_cat).toarray()
enc_svflat = pd.DataFrame(data=svflat_cat, columns=merged_enc_cat)
svflat = svflat.join(enc_svflat)
svflat = svflat.drop(['number_of_rooms'], axis=1)
svflat.to_excel('tables\svflat_dataframe.xlsx')
                     #           Train_Test_Split
...
norm_svflat = pd.DataFrame(scaler.transform(svflat[['total_square', 'living_square', 'kitchen_square', 'ceiling_height', 'house_age', 'distance', 'avg_sqrm_living_room']]), columns=['total_square', 'living_square', 'kitchen_square', 'ceiling_height', 'house_age', 'distance', 'avg_sqrm_living_room'])
norm_svflat = norm_svflat.join(svflat[['living_to_total', 'kitchen_to_living', 'kitchen_to_total', 'curFlor_to_totFlor']])
norm_svflat = norm_svflat.join(svflat[svflat.columns[-24:]])
norm_svflat.to_excel('tables\_normalized_svflat.xlsx')
